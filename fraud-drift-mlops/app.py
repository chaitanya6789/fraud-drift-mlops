...
st.markdown(
    "**Event-Driven Feature-Partitioned Sub-Ensemble Selective Retraining**"
)


# Recursive directory search to locate model artifacts automatically
...
uploaded_file = st.sidebar.file_uploader(
    "Upload Transaction Batch (CSV)", type=["csv"]
)
selected_partition = st.sidebar.selectbox(
    "Inject Drift into Feature Group",
    [
        "Partition 3 (Sub-Model 3)",
        "Partition 1 (Sub-Model 1)",
        "Partition 2 (Sub-Model 2)",
    ],
)
...
with tab1:
  ...
  st.markdown("##### **Load Test Simulation Preset:**")
  preset_col1, preset_col2, preset_col3 = st.columns(3)

  default_v1, default_v10, default_v12, default_v14, default_v17, default_v20, default_amt = (
      0.0,
      0.0,
      0.0,
      0.0,
  ...
  preset = st.radio(
      "Choose a Transaction Profile:",
      [
          "Standard Genuine Transaction",
          "High Velocity Anomaly (Drifted)",
          "High-Risk Adversarial Fraud",
      ],
      horizontal=True,
  )

  if preset == "Standard Genuine Transaction":
    default_v1, default_v10, default_v12, default_v14, default_v17, default_v20, default_amt = (
        0.5,
        0.2,
        0.4,
  ...
  elif preset == "High Velocity Anomaly (Drifted)":
    default_v1, default_v10, default_v12, default_v14, default_v17, default_v20, default_amt = (
        -1.2,
        -0.8,
        -1.0,
  ...
  elif preset == "High-Risk Adversarial Fraud":
    default_v1, default_v10, default_v12, default_v14, default_v17, default_v20, default_amt = (
        -4.8,
        -4.5,
        -4.9,
  ...
  with col1:
    ...
    v1 = st.slider("V1 (Profile Habit)", -5.0, 5.0, default_v1)
  ...
  with col2:
    ...
    v10 = st.slider("V10 (Channel Trust)", -5.0, 5.0, default_v10)
    v12 = st.slider("V12 (Token Validity)", -5.0, 5.0, default_v12)
    v14 = st.slider("V14 (Security Key)", -5.0, 5.0, default_v14)
    v17 = st.slider("V17 (Gateway Signature)", -5.0, 5.0, default_v17)
    g2_input = [v10, 0.0, v12, 0.0, v14, 0.0, 0.0, v17, 0.0, 0.0]

  with col3:
    st.markdown("##### **Partition 3: Amount & Velocity (V20–V28)**")
    raw_amt = st.number_input(
        "Transaction Amount ($/₹)", 0.0, 50000.0, default_amt
    )
    v20 = st.slider("V20 (Transaction Frequency)", -5.0, 5.0, default_v20)

  use_retrained = st.toggle(
      "Active Model Registry: Use Retrained Sub-Model 3 (Strategy C)",
      value=True,
  )
  ...
  if models_ready:
    ...
    if st.button("Evaluate Transaction Risk", type="primary"):
      ...
      if final_risk >= 0.50:
        st.error(
            f"🚨 **PREDICTION: FRAUD DETECTED (Risk: {final_risk * 100:.2f}%) —"
            " Transaction Intercepted & Blocked!**"
        )
      else:
        st.success(
            "🟢 **PREDICTION: GENUINE TRANSACTION (Risk:"
            f" {final_risk * 100:.2f}%) — Cleared & Approved.**"
        )
  else:
...
with tab2:
  ...
  benchmark_df = pd.DataFrame([
      {
          "MLOps Strategy": "Strategy A: Static Model (No Retraining)",
          "Precision": "0.9130",
          "Recall": "0.7500",
          "F1-Score": "0.8235",
          "ROC-AUC": "0.9535",
          "Compute Latency": "0.0000 s",
      },
      {
          "MLOps Strategy": "Strategy B: Global Retraining (29 Features)",
          "Precision": "1.0000",
          "Recall": "1.0000",
          "F1-Score": "1.0000",
          "ROC-AUC": "1.0000",
          "Compute Latency": "5.7067 s",
      },
      {
          "MLOps Strategy": "Strategy C: Proposed Selective Retraining",
          "Precision": "0.9038",
          "Recall": "0.8393",
          "F1-Score": "0.8704",
          "ROC-AUC": "0.9937",
          "Compute Latency": "0.4623 s",
      },
  ])
  st.table(benchmark_df)
  st.info(
      "⚡ **Total Cloud Compute & Execution Latency Reduction: 91.90%**"
      " (0.4623 s vs 5.7067 s)"
  )
  ...
  with fig_col2:
    ...
    if os.path.exists(img_p2):
      st.image(
          img_p2, caption="Fig 2: Baseline Sub-Model vs Ensemble Performance"
      )
    if os.path.exists(img_p4):
      st.image(
          img_p4,
          caption=(
              "Fig 4: Master Evaluation Suite (Latency & Confusion Matrix)"
          ),
      )
